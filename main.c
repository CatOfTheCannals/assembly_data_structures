...
void coutList(list_t *l) {
    listElem_t *nextNode = l->first;
    printf("[");
    while(nextNode != 0) {
        printf("%s", (char *)nextNode->data);
        if(!nextNode->data) printf("nodo vaciooo ");
...
void coutListReverse(list_t *l) {
    listElem_t *prevNode= l->last;
    printf("[");
    while(prevNode != 0) {
        printf("%s", (char *)prevNode->data);
        if(prevNode->prev != NULL) printf(", ");
        if(!prevNode->data) printf("nodo vaciooo ");
...
void test_0(){
    ...
    for(int i = 0; i<3; i++) {
        list_t *l;
        nTableAdd(n, i, strClone("holi"), (funcCmp_t*)&strCmp);
        nTableAdd(n, i, strClone("perinola"), (funcCmp_t*)&strCmp);
        l= n->listArray[i];
        coutList(l);
        coutListReverse(l);
    }
...
void coutArrayPointers(nTable_t* t) {
    uint32_t i = 0;
    while(i < t->size) {
        void *l = t->listArray[i];
        printf("(%d, %p)\n",i, l);
        i++;
    }
    printf("\n\n");
...
void test_3(char* filename){
    ...
    for(int s=0;s<40;s++) { 
        for(int i=0;i<1000;i++) {  
                char * rstr = randomString(rand()%10+1); 
                listAdd(l, rstr, (funcCmp_t*)&strCmp);
